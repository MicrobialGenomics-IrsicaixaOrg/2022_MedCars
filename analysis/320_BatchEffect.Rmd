...
metar::get_meta(myMREObject) %>%
  dplyr::filter(! SampleID %in% metadata_df$SampleID)
...
metadata_df %>% 
  ...
  dplyr::filter(Run=="Run3") %>% 
  dplyr::filter(Center=="C01") %>% 
  ggstatsplot::ggwithinstats(.,x=Timepoint,type = "nonparametric",
...
myMREObject@metadata@metadata_df<-metadata_df %>% as_tibble()
rownames(metadata_df)<-metadata_df$SampleID
sample_data(myMREObject@taxa@dada2@phyloseq)<-metadata_df

myMREObject@metadata@categorical_vals<-tibble::tribble(~CategoricalVariable,~PaletteName,"Run", "Accent")

myMREObject<-myMREObject %>% 
  dada2_nmds(tax_level="Genus",save_files=F)
...
myMREObject@taxa@metaphlan@phyloseq %>% 
  phyloseq::tax_glom(.,taxrank="Phylum") %>% 
  microbiome::transform(.,transform="compositional") %>%
  phyloseq::psmelt() %>% 
  ggstatsplot::ggbetweenstats(.,x=Run,y=Abundance,)
