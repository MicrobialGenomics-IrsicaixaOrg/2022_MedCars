...
Let's see how Month6 vs baseling comparisong are consistent in different runs. This would indicate a batch effect on alpha diversity that could be affecting our design. Note that it would not be affecting Delta comparison testing, since we can reasonably assume that Month6 vs. BL differences are stable.
...
### univariate model with no adjustment tells us that C02 has a slight higher shannon diversity.
...
aux_df %>%
  ...
  lm(ratio ~  Center+Gender+Age+Smoker+Run+UBE+IPAQ+MDScore+BMI,data=.)%>% broom::tidy() %>% knitr::kable(
    caption = "Multivariate Linear model for F/B ratio, adjusted for Age, Smoking, Alcohol Consumption and Sequencing Run.",
    col.names = c("Predictor", "B", "SE", "t", "p"),
    digits = c(0, 2, 3, 2, 4)
  )
```
