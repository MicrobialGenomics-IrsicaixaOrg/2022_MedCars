...
metar::get_meta(myMREObject) %>% dplyr::select(Visita)

metadata_df<-read.xlsx("data/raw/Metadata/FullMetadata_v1.0.xlsx",sheetIndex = 1) %>% 
...
metadata_df<-metadata_df %>% 
  ...
  dplyr::left_join(metadata_df,by=c("Timepoint","Participante")) %>% 
  dplyr::select(-visitDate) %>% 
  dplyr::rename(visitDate=Date)

metadata_df<-metadata_df %>% 
  dplyr::right_join(metar::get_meta(myMREObject),by=c("Participante","Center","Timepoint")) %>% 
...
phylum_df<-metar::get_phyloseq(myMREObject,type="dada2") %>% 
  ...
  dplyr::left_join(metadata_df %>%
                     dplyr::select(c("Center","Participante","Timepoint","visitDate")),by=c("Center","Participante","Timepoint")) %>%
  dplyr::filter(Timepoint=="BL")
...
phylum_df %>% 
  ...
  ggplot(.,aes(x=visitDate,y=Abundance))+geom_point()
```
