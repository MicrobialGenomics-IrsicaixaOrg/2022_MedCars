...
In this Analysis section we aim to describe possible differences between the two pariticpating centers at enf of study, in order to assess whether there are any and if they are relevant in relation to the longtiduinal resultes obtained from the nutritional intervention.
...
There are significant differences between groups at month6 in the unadjusted analysis, similary to baseline. Control Group(C02) shows a higher richness, evenness and phylogenetic diversity than intervention group at baseline. Reasons for this are unknown and may be due to socio-geographical features of each population. 
...
p0<-phyloseq::sample_data(ps_silva) %>% 
  ...
  ggstatsplot::ggbetweenstats(.,x=Center,
                              ...
                              p.adjust.method = "BH",pairwise.display = F, title =  "Chao1 vs Center / Month6",
...
p1<-phyloseq::sample_data(ps_silva) %>% 
  ...
  ggstatsplot::ggbetweenstats(.,x=Center,
                              ...
                              type = "parametric", 
                              centrality.plotting=T,bf.message = FALSE,
                              p.adjust.method = "BH", pairwise.display = F, title =  "Shannon  vs Center / Month6",
...
p2<-phyloseq::sample_data(ps_silva) %>% 
  ...
  ggstatsplot::ggbetweenstats(.,x=Center,
                              ...
                              type = "parametric", 
                              centrality.plotting=T,bf.message = FALSE,
                              p.adjust.method = "BH", pairwise.display = F, title =  "PD  vs Center / Month6",
                              annotation.args = list(title = "Shannon index vs Timepoint",
                                                         theme = ggplot2::theme(
                                                                plot.subtitle = ggplot2::element_text(size = 13),
                                                                plot.title = ggplot2::element_text(size = 14)
...
myMREObject<-readRDS("data/cfu/mreObject.rds")
### Month6
Month6_samples<-myMREObject %>% 
  metar::get_meta() %>% 
  dplyr::filter(Timepoint=="Month6") %>% 
  dplyr::pull(SampleID) %>% as.vector()

auxMREObject<-myMREObject %>%
  metar::filter_samples(.,Month6_samples) 
...
auxMREObject@taxa@dada2@nmds$Genus$top_50$all_adonis
...
Results indicate that two bacterial genera are over  abundant in the Control Group(C02) vs the Intervention Group(C01), when comparing baseline(Month6)  samples: Lachnospira and Holdemanella. 
...
p0<-aux_res %>% 
    ...
    theme_classic()+ggtitle("C01 vs C02 | Month6")+
    scale_fill_brewer(palette="Dark2") 
    ggsave(plot=p0,"output/Deliverables/DifferentialAbundance/lefse/boxplots/lefser_C01_vs_C02_Month6_samples.pdf")
...
for(otu_name in aux_res$otu){
  ...
  ggstatsplot::ggbetweenstats(data = .,x=Center,y=Abundance,
   ...
   ggsave(.,filename=paste0("output/Deliverables/DifferentialAbundance/lefse/boxplots/",title$Names,"_vs_Center_Month6_Samples.pdf"),width=7, height=7)
}
...
```{r DA species maaslin2 Month6, include=TRUE, echo=F, message=FALSE, warning=FALSE, paged.print=FALSE, tidy=T,eval=T}
...
input_data<-myMREObject@taxa@dada2@phyloseq %>% 
  phyloseq::subset_samples(Timepoint=="Month6") %>% tax_glom(.,taxrank="Genus",NArm = F) %>% otu_table() %>% t()
...
suppressWarnings(fit_data <- Maaslin2::Maaslin2(input_data=input_data,
                                                ...
                                                output="output/Deliverables/DifferentialAbundance/maaslin/genusMonth6",
                                                fixed_effects = c("Center"),
                                                random_effects = c("Gender","UBE","Edad","Run"),
