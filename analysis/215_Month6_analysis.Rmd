...
There are significant differences between groups at month6 in the unadjusted analysis, similary to baseline. Control Group(CG) shows a higher richness, evenness and phylogenetic diversity than intervention group at baseline. Reasons for this are unknown and may be due to socio-geographical features of each population. 
...
p0<-phyloseq::sample_data(ps_silva) %>% 
  ...
  ggstatsplot::ggbetweenstats(.,x=Center,
                              ...
                              p.adjust.method = "BH",pairwise.display = F, title =  "Chao1 vs Center / Month8",
...
p1<-phyloseq::sample_data(ps_silva) %>% 
  ...
  ggstatsplot::ggbetweenstats(.,x=Center,
                              ...
                              type = "parametric", 
                              centrality.plotting=T,bf.message = FALSE,
                              p.adjust.method = "BH", pairwise.display = F, title =  "Shannon  vs Center / Month8",
...
p2<-phyloseq::sample_data(ps_silva) %>% 
  ...
  ggstatsplot::ggbetweenstats(.,x=Center,
                              ...
                              type = "parametric", 
                              centrality.plotting=T,bf.message = FALSE,
                              p.adjust.method = "BH", pairwise.display = F, title =  "PD  vs Center / Month8",
                              annotation.args = list(title = "Shannon index vs Timepoint",
                                                         theme = ggplot2::theme(
                                                                plot.subtitle = ggplot2::element_text(size = 13),
                                                                plot.title = ggplot2::element_text(size = 14)
...
myMREObject<-readRDS("data/cfu/mreObject.rds")
### Month8
Month8_samples<-myMREObject %>% 
  metar::get_meta() %>% 
  dplyr::filter(Timepoint=="Month8") %>% 
  dplyr::pull(SampleID) %>% as.vector()

auxMREObject<-myMREObject %>%
  metar::filter_samples(.,Month8_samples) 
...
Results indicate that two bacterial genera are over  abundant in the Control Group(CG) vs the Intervention Group(IG), when comparing baseline(Month8)  samples: Lachnospira and Holdemanella. 
...
p0<-aux_res %>% 
    ...
    theme_classic()+ggtitle("IG vs CG | Month8")+
    scale_fill_brewer(palette="Dark2") 
    ggsave(plot=p0,"output/Deliverables/DifferentialAbundance/lefse/boxplots/lefser_IG_vs_CG_Month8_samples.pdf")
...
for(otu_name in aux_res$otu){
  ...
  ggstatsplot::ggbetweenstats(data = .,x=Center,y=Abundance,
   ...
   ggsave(.,filename=paste0("output/Deliverables/DifferentialAbundance/lefse/boxplots/",title$Names,"_vs_Center_Month8_Samples.pdf"),width=7, height=7)
}
...
```{r DA species maaslin2 Month8, include=TRUE, echo=F, message=FALSE, warning=FALSE, paged.print=FALSE, tidy=T,eval=T}
...
input_data<-myMREObject@taxa@dada2@phyloseq %>% 
  phyloseq::subset_samples(Timepoint=="Month8") %>% tax_glom(.,taxrank="Genus",NArm = F) %>% otu_table() %>% t()
...
suppressWarnings(fit_data <- Maaslin2::Maaslin2(input_data=input_data,
                                                ...
                                                output="output/Deliverables/DifferentialAbundance/maaslin/genusMonth8",
